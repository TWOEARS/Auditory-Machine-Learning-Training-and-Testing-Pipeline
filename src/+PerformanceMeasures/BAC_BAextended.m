...
%                 agPis2 = PerformanceMeasures.BAC_BAextended.baParams2bapIdxs( agBap2 );
        ...
        function [baParams, asgn] = extractBAparams( blockAnnotations, scp, yp, yt )
            ...
            nonemptybaf2 = ~cellfun( @isempty, bafFilesepIdxs );
            bafClasses = cellfun( @(fp,idx)(fp(idx(end-1)+1:idx(end)-1)), bafiles(nonemptybaf2), bafFilesepIdxs(nonemptybaf2), 'UniformOutput', false );
            [~,classIdxs] = PerformanceMeasures.BAC_BAextended.classList;
            bafClassIdxs = cellfun( @(c)(classIdxs.(c)), bafClasses, 'UniformOutput', false );
            
			nonemptybaf(nonemptybaf) = nonemptybaf2;
        ...
        function collectBAEresults( testmat, cc, scps )
            
            startAMLTTP();
            addPathsIfNotIncluded( fullfile( pwd, '..' ) );
            ...
            if exist( [testmat '.mat'], 'file' )
                filesema = setfilesemaphore( [testmat '.mat'] );
                fprintf( '-' );
                load( [testmat '.mat'] );
                cacheFileInfo = dir( [testmat '.mat'] );
            ...
            if nargin < 3 || isempty( scps ), scps = unique( cellfun( @(c)(c(2)), doneCfgsTest ) ); end
            ...
            for ll = cc
                ...
                for dcidx = 1 : numel( teCfgs )
                    ...
                    fprintf( '\n============== collect BAext results %s; ll = %d, scp = %d ==============\n', ...
                        testmat, ll, scp );
                ...
                filesema = setfilesemaphore( [testmat '.mat'] );
                if exist( [testmat '.mat'], 'file' )
                    newCacheFileInfo = dir( [testmat '.mat'] );
                    if ~isempty( newCacheFileInfo ) && ~isequalDeepCompare( newCacheFileInfo, cacheFileInfo )
                        fileupdate = load( [testmat '.mat'] );
                        doneCfgsTest = fileupdate.doneCfgsTest;
                        modelpathes_test = fileupdate.modelpathes_test;
                        test_performances_b = fileupdate.test_performances_b; %#ok<NASGU>
                        fprintf( ':' );
                        rescMd_b = syncResults2( rescMd_b, fileupdate.rescMd_b, cc, 2 );
                        fprintf( ':' );
                        rescMd_t = syncResults2( rescMd_t, fileupdate.rescMd_t, cc, 2 );
                        fprintf( ':' );
                        rescMd_t2 = syncResults2( rescMd_t2, fileupdate.rescMd_t2, cc, 2 );
                    end
                    fprintf( ':' );
                end
                save( [testmat '.mat'], ...
