classdef TwoEarsIdTrainPipe < handle
    ...
    methods
        ...
        function init( obj, sceneCfgs, varargin )
            ...
            bsWrapperIdxAdd = double( ~isempty( obj.blackboardSystem ) );
            if pipeReUseIdx < 4+ksWrapperIdxAdd+bsWrapperIdxAdd ...
                    || numel( obj.pipeline.dataPipeProcs ) < 4+ksWrapperIdxAdd+bsWrapperIdxAdd ...
                    || isempty( obj.pipeline.dataPipeProcs{4+ksWrapperIdxAdd+bsWrapperIdxAdd} )
				...
				if ~isempty( obj.blackboardSystem )
                    multiCfgProcs{3} = DataProcs.MultiSceneCfgsIdProcWrapper( ...
                                      obj.binSim, obj.blackboardSystem, [], wavFoldAssignments );
				end
                ...
                multiCfgProcs{3+bsWrapperIdxAdd} = DataProcs.MultiSceneCfgsIdProcWrapper( ...
                                   obj.binSim, obj.blockCreator, [], wavFoldAssignments );
                if ~isempty( obj.ksWrapper )
                    multiCfgProcs{4+bsWrapperIdxAdd} = DataProcs.MultiSceneCfgsIdProcWrapper( ...
                                      obj.binSim, obj.ksWrapper, [], wavFoldAssignments );
                end
                multiCfgProcs{4+ksWrapperIdxAdd+bsWrapperIdxAdd} = ...
                    DataProcs.MultiSceneCfgsIdProcWrapper( ...
                                 obj.binSim, obj.featureCreator, [], wavFoldAssignments );
                obj.pipeline.resetDataProcs( 2 );
            end
            if pipeReUseIdx < 5+ksWrapperIdxAdd+bsWrapperIdxAdd ...
                    || numel( obj.pipeline.dataPipeProcs ) < 5+ksWrapperIdxAdd+bsWrapperIdxAdd ...
                    || isempty( obj.pipeline.dataPipeProcs{5+ksWrapperIdxAdd+bsWrapperIdxAdd} )
                multiCfgProcs{5+ksWrapperIdxAdd+bsWrapperIdxAdd} = ...
                    DataProcs.MultiSceneCfgsIdProcWrapper( ...
                                   obj.binSim, obj.labelCreator, [], wavFoldAssignments );
                obj.pipeline.resetDataProcs( 5+ksWrapperIdxAdd+bsWrapperIdxAdd );
            end
            if useGatherFeaturesProc && ...
                    ( numel( obj.pipeline.dataPipeProcs ) < 6+ksWrapperIdxAdd+bsWrapperIdxAdd ...
                      || isempty( obj.pipeline.dataPipeProcs{6+ksWrapperIdxAdd+bsWrapperIdxAdd} ) )
                ...
                multiCfgProcs{6+ksWrapperIdxAdd+bsWrapperIdxAdd} = ...
                    DataProcs.MultiSceneCfgsIdProcWrapper( ...
