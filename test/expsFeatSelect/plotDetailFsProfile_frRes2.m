function plotDetailFsProfile_frRes2( featureNames, impacts, addTitle )
...
for ii = 1 : numel( fGrps )
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'ratemap','16-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsRMbl1 = [fsRMbl1 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'amsFeatures','8-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsAMbl1 = [fsAMbl1 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'onsetStrength','16-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsOMbl1 = [fsOMbl1 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    fprintf('.');
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'ratemap','48-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsRMbl2 = [fsRMbl2 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'24-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsAMbl2 = [fsAMbl2 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'onsetStrength','48-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
...
hold all;
hPlot = bar( log10(binr), [bincRMbl1',bincRMbl2',...
                           bincOMbl1',bincOMbl2',...
                           bincAMbl1',bincAMbl2'], ...
...
legend( {'ratemap 16-ch', 'ratemap 48-ch',...
         'onsetStrength 16-ch', 'onsetStrength 48-ch',...
         'amplitude modulation 8-ch', 'amplitude modulation 24-ch'}, 'Location', 'Best' );
