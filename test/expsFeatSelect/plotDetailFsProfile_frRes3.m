function plotDetailFsProfile_frRes3( featureNames, impacts, addTitle )
...
for ii = 1 : numel( fGrps )
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'8-ch'}, {fGrps{ii},'16-ch'}, {fGrps{ii},'32-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
    fsbl1 = [fsbl1 ones(1,round(grpImpact*1000))*fGrpsNum(ii)];
    grpIdxs = getFeatureIdxs( featureNames, {fGrps{ii},'24-ch'}, {fGrps{ii},'48-ch'}, {fGrps{ii},'64-ch'} );
    grpImpact = sum( impacts(grpIdxs) );
    nTotal = nTotal + sum( impacts(grpIdxs) > 0 );
...
hold all;
hPlot = bar( log10(binr), [bincbl1',bincbl2'], ...
