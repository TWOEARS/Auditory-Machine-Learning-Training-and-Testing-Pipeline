...
for fc = 1 : 3
for ll = 1 : 3
    tp_mc(2+ll,fc,cc) = test_performances(fc,ll,1);
end
    tp_mc(6,fc,cc) = test_performances(fc,3,2);
end
end
...
figure( 'Name', 'mc train set; mc test set', 'position', [0, 0, 900, 600] )
hold all;
...
plot( [1, 2, 3, 4], [tp_mc{6,1,:}], 'oy', 'DisplayName','fs3-svm-rbf, monaural', 'LineWidth', 3 );
...
plot( [0.90, 1.90, 2.90, 3.90], [tp_mc{6,2,:}], 'sy', 'DisplayName','fs3-svm-rbf, varBlockLengths', 'LineWidth', 3 );
...
plot( [1.10, 2.10, 3.10, 4.10], [tp_mc{6,3,:}], 'dy', 'DisplayName','fs3-svm-rbf, varFreqRes', 'LineWidth', 3 );
ylabel( 'performance' );
...
set( gca, ...
     'XTickLabel',...
        {'monaural',...
         ...
         'YLim', [0.8 1] );
legend( {'fs1-lasso', 'fs3-lasso', 'O-svm', 'fs1-svm', 'fs3-svm', 'fs3-svm-rbf'},'Location','Best' );

savePng( 'mc train mc test,classes mean,algs' );

figure( 'Name', 'all fcs mean. mc train set; mc test set' )
hold all;
bar( squeeze( cellSqueezeFun( @(cc)(mean([cc{:}])), tp_mc(:,:,:), 2 ) )' );
ylabel( 'test performance' );
set( gca, ...
     'XTickLabel',...
        {'alarm',...
         'baby',...
         'female', 'fire' }, 'XTick',[1 2 3 4],...
         'YLim', [0.8 1] );
legend( {'fs1-lasso', 'fs3-lasso', 'O-svm', 'fs1-svm', 'fs3-svm', 'fs3-svm-rbf'},'Location','Best' );

boxplot_performance( ...
    'algorithm performance comparison',...
    {'fs1-lasso','fs3-lasso','O-svm','fs1-svm','fs3-svm','fs3-svm-rbf'},...
    [1 1 2 2 2 3],...
...
boxplot_performance( ...
    'algorithm performance comparison',...
    {'fs1-lasso - fs1-svm','fs1-svm - O-svm','fs3-svm-rbf - fs3-svm','fs3-svm-rbf - fs1-svm'},...
    [1 1 1 1],...
    {'notch','off'},...
    reshape(cell2mat(tp_mc(1,:,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(4,:,[1,2,4])),1,[]),...
    reshape(cell2mat(tp_mc(4,:,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(3,:,[1,2,4])),1,[]),...
    reshape(cell2mat(tp_mc(6,:,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(5,:,[1,2,4])),1,[]),...
    reshape(cell2mat(tp_mc(6,:,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(4,:,[1,2,4])),1,[]) ); 
ylim( [-0.05,0.05] )

boxplot_performance( ...
    'fc performance comparison',...
    {'VarBlockLengths - Monaural','Monaural - VarFreqRes','VarBlockLengths - VarFreqRes'},...
    [1 1 1],...
    {'notch','off'},...
    reshape(cell2mat(tp_mc(:,2,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(:,1,[1,2,4])),1,[]),...
    reshape(cell2mat(tp_mc(:,1,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(:,3,[1,2,4])),1,[]),...
    reshape(cell2mat(tp_mc(:,2,[1,2,4])),1,[])-reshape(cell2mat(tp_mc(:,3,[1,2,4])),1,[]) ); 
ylim( [-0.05,0.05] )

boxplot_performance( ...
    'mc train sc test;all classes,all fcs,glmnet&svm -fs1',...
    {...
     'GO: -45/45°, 20dB (cross)',...
     'GO: 0/90°, 10dB (cross)',...
     'GO: -90/90°, 0dB (iso)',...
     ...
     [1 1 2 2 2 2 1 1 3 4 3 4 5 6 5 6 6 5],...
     {'notch','off','widths', 0.8},...
     [tp_mc_testOnSc{[1,4],:,1:3,12}],...
     [tp_mc_testOnSc{[1,4],:,1:3,11}],...
     [tp_mc_testOnSc{[1,4],:,1:3,4}],...
     [tp_mc_testOnSc{[1,4],:,1:3,3}],...
     [tp_mc_testOnSc{[1,4],:,1:3,2}],...
     [tp_mc_testOnSc{[1,4],:,1:3,1}],...
     [tp_mc_testOnSc{[1,4],:,1:3,13}],...
     [tp_mc_testOnSc{[1,4],:,1:3,10}],...
     [tp_mc_testOnSc{[1,4],:,:,5}],...
     [tp_mc_testOnSc{[1,4],:,:,14}],...
     [tp_mc_testOnSc{[1,4],:,:,6}],...
     [tp_mc_testOnSc{[1,4],:,:,15}],...
     [tp_mc_testOnSc{[1,4],:,:,16}],...
     [tp_mc_testOnSc{[1,4],:,:,9}],...
     [tp_mc_testOnSc{[1,4],:,:,18}],...
     [tp_mc_testOnSc{[1,4],:,:,8}],...
     [tp_mc_testOnSc{[1,4],:,:,7}],...
     [tp_mc_testOnSc{[1,4],:,:,17}] ...
     );
