...
if exist( 'glmnet_azms_test1.mat', 'file' )
    altmat = load( 'glmnet_azms_test1.mat' );
end
...
if exist( 'modelpathes_test','var' )  &&  ...
    ...
    if exist( 'altmat', 'var' ) && isfield( altmat, 'modelpathes_test' )  &&  ...
            size(altmat.modelpathes_test,1) >= ii  &&  size(altmat.modelpathes_test,2) >= cc  &&  ...
            size(altmat.modelpathes_test,3) >= fc  &&  size(altmat.modelpathes_test,4) >= aa  &&  ...
            size(altmat.modelpathes_test,5) >= aatest  ...
            &&  ~isempty( altmat.modelpathes_test{ii,cc,fc,aa,aatest} )
        modelpathes_test{ii,cc,fc,aa,aatest} = altmat.modelpathes_test{ii,cc,fc,aa,aatest};
    else
        continue;
    end
end
...
if exist( 'lambdas','var' )  &&  ...
        size(lambdas,1) >= ii  &&  size(lambdas,2) >= cc  &&  ...
        size(lambdas,3) >= fc  &&  size(lambdas,4) >= aa  &&  ...
        size(lambdas,5) >= aatest  ...
        &&  ~isempty( lambdas{ii,cc,fc,aa,aatest} )
    continue;
end
...
[lambdas{ii,cc,fc,aa,aatest},...
 nCoefs{ii,cc,fc,aa,aatest},...
 cumImpacts{ii,cc,fc,aa,aatest}] = testmodel.model.getLambdasAndNCoefs();
