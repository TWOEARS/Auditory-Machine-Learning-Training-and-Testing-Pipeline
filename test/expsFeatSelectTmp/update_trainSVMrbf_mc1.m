function update_trainSVMrbf_mc1()
...
if exist( 'modelpathes_svm','var' )  &&  ...
    ...
    if exist( 'altmat', 'var' ) && isfield( altmat, 'modelpathes_svm' )  &&  ...
        ...
        size(altmat.modelpathes_svm,3) >= kk  &&  ~isempty( altmat.modelpathes_svm{fc,ll,kk} )
        modelpathes_svm{fc,ll,kk} = altmat.modelpathes_svm{fc,ll,kk};
        if isfield( altmat, 'test_performances' )  &&  ...
                size(altmat.test_performances,1) >= fc  &&  size(altmat.test_performances,2) >= ll  &&  ...
                size(altmat.test_performances,3) >= kk  &&  ~isempty( altmat.test_performances{fc,ll,kk} )
            test_performances{fc,ll,kk} = altmat.test_performances{fc,ll,kk};
        end
    else
        continue;
    end
...
if exist( 'modelpathes_svm','var' )  &&  ...
        (size(modelpathes_svm,1) < fc  ||  size(modelpathes_svm,2) < ll)  ||  size(modelpathes_svm,3) < kk)
    if exist( 'altmat', 'var' ) && isfield( altmat, 'modelpathes_svm' )  &&  ...
            size(altmat.modelpathes_svm,1) >= fc  &&  size(altmat.modelpathes_svm,2) >= ll  &&  size(altmat.modelpathes_svm,3) >= kk  ...
            &&  ~isempty( altmat.modelpathes_svm{fc,ll,kk} )
        modelpathes_svm{fc,ll,kk} = altmat.modelpathes_svm{fc,ll,kk};
        if isfield( altmat, 'test_performances' )  &&  ...
                size(altmat.test_performances,1) >= fc  &&  size(altmat.test_performances,2) >= ll  &&  size(altmat.test_performances,3) >= kk  ...
                &&  ~isempty( altmat.test_performances{fc,ll,kk} )
            test_performances{fc,ll,kk} = altmat.test_performances{fc,ll,kk};
        end
    else
        continue;
...
if exist( 'test_performances','var' )  &&  ...
        size(test_performances,1) >= fc  &&  size(test_performances,2) >= ll  &&  size(test_performances,3) >= kk  ...
        &&  ~isempty( test_performances{fc,ll,kk} )
    continue;
end
    
m = load( [modelpathes_svm{fc,ll,kk} filesep classname '.model.mat'] );
...
pipe.modelCreator = ...
    modelTrainers.LoadModelNoopTrainer( ...
        @(cn)(fullfile( modelpathes_svm{fc,ll,kk}, [cn '.model.mat'] )), ...
...
test_performances{fc,ll,kk} = [testmodel.testPerfresults.performance];
